...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, mask=None, return_attention=False):
        if return_attention:
            attn_out, attn_weights = self.attention(x, x, x, mask, return_attention=True)
        else:
...
class CustomTransformerLM(nn.Module):
    ...
    def forward(self, idx, targets=None, return_attention=False):
        ...
        if targets is not None:
            # ignore_index=1 — не учить модель предсказывать <UNK> (token ID 1)
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None):
        ...
        for _ in range(max_new_tokens):
            ...
            # Подавляем <UNK> (ID=1) и <PAD> (ID=0) — модель не должна их генерировать
