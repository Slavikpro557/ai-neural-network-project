    ...
    <div class="container">
        ...
        <div id="catalog" class="tab-content">
            ...
            <div class="search-box" style="margin-top:10px;">
                <input type="text" id="custom_url_name" placeholder="Название" style="flex:0.3;">
        ...
        <div id="train" class="tab-content">
            ...
            <div class="status-box">
                ...
                <div class="status-item"><span class="status-label" data-i18n="status_label">Статус:</span><span class="status-value" id="is_training">Не запущено</span></div>
        ...
        <div id="compare" class="tab-content">
            ...
            <div class="form-group"><label>Модель:</label><select id="compare_model_name" onchange="loadCheckpoints()"></select></div>
            <div class="grid">
                <div class="form-group"><label>Чекпоинт A:</label><select id="checkpoint_a"></select></div>
                <div class="form-group"><label>Чекпоинт B:</label><select id="checkpoint_b"></select></div>
            </div>
            <div class="form-group"><label>Промпт:</label><input type="text" id="compare_prompt" value="Искусственный интеллект"></div>
...
async function previewDS(id) {
    ...
    const data = await resp.json();
    document.getElementById('preview_title').textContent = data.name || 'Превью';
...
async function loadHWRecommendation() {
    try {
        ...
        const data = await resp.json();
        document.getElementById('hw_recommendation').innerHTML = '<div class="alert alert-info">Рекомендация: <strong>' + data.preset.toUpperCase() + '</strong> — ' + data.reason + '</div>';
    } catch(e) {}
}
...
async function uploadBook() {
    ...
    try {
        ...
        if (data.status === 'success') {
            document.getElementById('upload_status').innerHTML = '<div class="alert alert-success">Загружено: ' + data.filename + ' (' + (data.size/1024).toFixed(1) + ' KB)</div>';
            loadDatasets();
        }
